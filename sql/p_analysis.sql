...
SELECT industry, sum(amount) / (SELECT SUM(amount) FROM asset_management_p.transactions) as prc
FROM asset_management_p.transactions
GROUP BY industry
;

-- This query checks that "prc" column sums to 1.
SELECT SUM(prc) FROM (SELECT industry, sum(amount) / (SELECT SUM(amount) FROM asset_management_p.transactions) as prc
FROM asset_management_p.transactions
